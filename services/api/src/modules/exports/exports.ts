...
import { canExportScope } from "../../shared/permissions.js";
...
router.get("/:exportId/download-url", async (request: AuthenticatedRequest, response) => {
  ...
  if (!access.canExport) {
    response.status(403).json({ error: "You need owner, admin, or manager access to download exports." });
    return;
  }
...
async function getProjectExportAccess(userId: string, project: ExportScopeProject) {
  ...
  const canExport = canExportScope({
    isProjectCreator: project.createdById === userId,
...
function buildJsonMinExport(payload: ApprovedAnnotationsExportPayload) {
  return payload.tasks.map((task) => {
    ...
    if (firstAnnotation?.regions.length) {
      record.regions = firstAnnotation.regions.map(serializeCompactRegion);
    }

    if (task.annotations.length > 1) {
      record.annotations = task.annotations.map((annotation) => ({
        ...getAnnotationControlValues(annotation),
        regions: annotation.regions.map(serializeCompactRegion)
      }));
    }
...
function serializeCompactRegion(region: ExportRegion) {
  return {
