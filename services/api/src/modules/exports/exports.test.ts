...
describe("buildApprovedAnnotationsExportPayload", () => {
  it("serializes file sizes and annotation counts for approved export payloads", () => {
    ...
    assert.equal(payload.tasks[0]?.annotations[0]?.regions[0]?.confidence, 0.91);
    assert.deepEqual(payload.tasks[0]?.annotations[0]?.regions[0]?.metadata, {
      aiJobId: "job-1",
      source: "ai_prediction"
    });
    ...
    const compact = JSON.parse(jsonMin.content.toString()) as Array<{ regions?: Array<{ confidence?: number; metadata?: unknown }> }>;

    assert.equal(compact[0]?.regions?.[0]?.confidence, 0.91);
    assert.deepEqual(compact[0]?.regions?.[0]?.metadata, {
      aiJobId: "job-1",
      source: "ai_prediction"
    });
  });
