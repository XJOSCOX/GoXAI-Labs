...
router.get("/", async (request: AuthenticatedRequest, response) => {
  ...
  const datasets = await prisma.dataset.findMany({
    ...
    include: getDatasetIncludes(user.id),
    orderBy: {
      updatedAt: "desc"
    }
  });
...
router.get("/:datasetId/versions", async (request: AuthenticatedRequest, response) => {
  ...
  const membership = await getEffectiveDatasetManagerMembership(user.id, dataset.projectId, dataset.organizationId);

  if (dataset.project.createdById !== user.id && (!membership || !canManageDatasets(membership))) {
    response.status(403).json({ error: "You need owner, admin, or manager access to view dataset versions." });
    return;
  }
...
router.post("/", async (request: AuthenticatedRequest, response) => {
  ...
  const membership = await getEffectiveDatasetManagerMembership(user.id, project.id, project.organizationId);

  if (project.createdById !== user.id && (!membership || !canManageDatasets(membership))) {
    response.status(403).json({
...
router.post("/:datasetId/versions/:version/rollback", async (request: AuthenticatedRequest, response) => {
  ...
  if (dataset.project.createdById !== user.id && (!membership || !canManageDatasets(membership))) {
    response.status(403).json({ error: "You need owner, admin, or manager access to rollback this dataset." });
    return;
  }
...
router.post("/:datasetId/restore", async (request: AuthenticatedRequest, response) => {
  ...
  if (dataset.project.createdById !== user.id && (!membership || !canManageDatasets(membership))) {
    response.status(403).json({ error: "You need owner, admin, or manager access to restore this dataset." });
    return;
  }
...
router.delete("/:datasetId", async (request: AuthenticatedRequest, response) => {
  ...
  if (dataset.project.createdById !== user.id && (!membership || !canManageDatasets(membership))) {
    response.status(403).json({ error: "You need owner, admin, or manager access to delete this dataset." });
    return;
  }
...
function getDatasetIncludes(currentUserId: string) {
  return {
  annotationTemplate: {
    select: {
      id: true,
...
type DatasetWithRelations = Prisma.DatasetGetPayload<{ include: ReturnType<typeof getDatasetIncludes> }>;

function serializeDataset(dataset: DatasetWithRelations, currentUserId?: string) {
  const projectMembership = currentUserId
    ? dataset.project.projectMemberships.find((item) => item.userId === currentUserId && item.status === "ACTIVE")
    : undefined;
  const organizationMembership = currentUserId
    ? dataset.organization.memberships.find((item) => item.userId === currentUserId && item.status === "ACTIVE")
    : undefined;
  const isProjectCreator = currentUserId === dataset.project.createdById;
  const canManageDataset = canManageDatasetScope({
    isProjectCreator,
    organizationMembership,
    projectMembership
  });
  const canManageDatasetAssets = canManageAssetScope({
    isProjectCreator,
    organizationMembership,
    projectMembership
  });
  const canGenerateDatasetTasks = canGenerateTaskScope({
    isProjectCreator,
    organizationMembership,
    projectMembership
  });
  const { projectMemberships: _projectMemberships, ...project } = dataset.project;
  const { memberships: _memberships, ...organization } = dataset.organization;
  ...
  return {
    ...
    counts: {
      ...
      taskCount: dataset._count.tasks
    },
    canManage: canManageDataset,
    canManageAssets: canManageDatasetAssets,
    canGenerateTasks: canGenerateDatasetTasks,
...
async function getEffectiveDatasetManagerMembership(userId: string, projectId: string, organizationId: string) {
  const prisma = getPrismaClient();
  ...
  return getEffectiveMembershipForCapability(organizationMembership, projectMembership, canManageDatasets);
}
